function fig4_ellipse
%plot figure 4 / a) & b)
%the ellipse constraint
video = 0;
if video
...
obj_fcn = @(W)      sum(W.^2,1);
constrain_fcn = @(W)    sum((W+[1;0]).^2./[2;1],1) - 1;
grad_constrain_fcn = @(W)   W.*[1;2]+[1;0];
Hess_constrain_fcn = @(W)   repmat([1,0;0,2],[1,1,size(W,2)]);
...
for simu = 1:simu_N
    ...
    for t = 1:tN %this is for full batch
        ...
        f(idx) = obj_fcn(W(:,idx))+lam *constrain_fcn(W).^2;
        ...
        er_record(t,simu) = norm(v_W - w_star)/sqrt(dim);
         ...
         if video
            ...
            xlim([-3,3]); ylim([-3,3]);
            x2 = linspace(-1,1,100); x1 = sqrt(2*(1-x2.^2))-1;
            plot(x1,x2,'k'); hold on; 
            x1 = -sqrt(2*(1-x2.^2))-1;
            plot(x1,x2,'k'); xlim([-3,3]); ylim([-3,3]);
            hold off;
            frame = getframe(gcf);
            writeVideo(v,frame);
